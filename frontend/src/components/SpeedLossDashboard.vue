<template>
  <div class="speed-loss-dashboard">
    ...
    <!-- 維修效益詳細表格 -->
    <section class="maintenance-detail-section">
      <h2>維修效益分析（含 RPM 正規化）</h2>
      <table class="maintenance-detail-table">
        <thead>
          <tr>
            ...
            <th>RPM 前</th>
            <th>RPM 後</th>
            <th>FOC 改善<br/>（30天平均）</th>
            <th>FOC 改善<br/>（RPM正規化）</th>
            <th>解讀</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="event in maintenanceEvents.slice(0, 15)" :key="`${event.ship_id}-${event.event_day}`">
            <td>{{ event.ship_id }}</td>
            <td>Day {{ event.event_day }}</td>
            <td>{{ formatEventType(event.event_type) }}</td>
            <td>{{ event.rpm_before_avg ? event.rpm_before_avg.toFixed(1) : '-' }}</td>
            <td>{{ event.rpm_after_avg ? event.rpm_after_avg.toFixed(1) : '-' }}</td>
            <td :class="getMaintInterpretClass(event.foc_drop_pct)">
              {{ event.foc_drop_pct ? event.foc_drop_pct.toFixed(2) + '%' : '-' }}
            </td>
            <td :class="getMaintInterpretClass(event.rpm_normalized_foc_drop_pct)">
              <strong>{{ event.rpm_normalized_foc_drop_pct ? event.rpm_normalized_foc_drop_pct.toFixed(2) + '%' : '-' }}</strong>
            </td>
            <td class="maint-interpret">{{ getMaintInterpretation(event) }}</td>
          </tr>
        </tbody>
      </table>
    </section>
...
export default {
  ...
  setup() {
    ...
    const bestShipLoss = computed(() => {
      if (!vizData.value) return '-'
      return (vizData.value.fleet_summary?.best_ship_sl || 0).toFixed(2)
    })

    const worstShipLoss = computed(() => {
      if (!vizData.value) return '-'
      return (vizData.value.fleet_summary?.worst_ship_sl || 0).toFixed(2)
    })

    const anomalyCount = computed(() => {
      if (!vizData.value) return 0
      return (vizData.value.maintenance_events || []).length
    })
    ...
    const loadData = async () => {
      try {
        ...
        console.log('✅ 已加載數據:', vizData.value)
      } catch (error) {
    ...
    const getMaintInterpretClass = (value) => {
      if (!value) return ''
    ...
    const getMaintInterpretation = (event) => {
      if (!event.rpm_normalized_foc_drop_pct) return '無法計算'
      const normalized = event.rpm_normalized_foc_drop_pct
      const raw = event.foc_drop_pct || 0

      // 比較 30 天平均和 RPM 正規化的差異
      if (normalized > 0 && raw > 0) {
        return '✅ 維修有效，油耗確實改善'
      } else if (normalized > 0 && raw < -10) {
        return '⚠️ 維修後提高轉速，但相同轉速確實改善'
      } else if (normalized > 0 && raw < 0) {
        return '✅ 維修有效，雖然轉速降低了'
      } else if (normalized < -5 && raw < -5) {
        return '❌ 維修可能無效，甚至變差'
      } else if (normalized > 0 && Math.abs(raw) < 5) {
        return '✅ 溫和改善'
      } else {
        return '⚠️ 混合信號'
      }
    ...
    return {
      ...
      formatEventType,
      formatAnomalyType,
      getMaintInterpretClass,
      getMaintInterpretation,
      formatValue,
      getHeatmapColor,
      investigateAnomaly,
      generateReport,
...
.maintenance-detail-table thead {
  background: #f8f9fa;
}

.maintenance-detail-table th {
  padding: 12px;
  text-align: left;
  font-weight: 600;
  color: #2c3e50;
...
.maintenance-detail-table td {
  padding: 12px;
  border-bottom: 1px solid #ecf0f1;
}

.maintenance-detail-table td.positive {
  color: #27ae60;
  font-weight: 600;
}

.maintenance-detail-table td.negative {
  color: #e74c3c;
  font-weight: 600;
}

.maintenance-detail-table td.neutral {
  color: #95a5a6;
}
