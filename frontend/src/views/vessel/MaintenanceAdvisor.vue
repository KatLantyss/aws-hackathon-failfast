...
<template>
  <div class="flex flex-col gap-4">
    <StateDisplay
      ...
      empty-title="Ê≠§ËàπÂ∞öÁÑ°Á∂≠‰øÆÂª∫Ë≠∞Ë≥áÊ?"
    />
    <template v-else-if="data">
      <div class="panel p-3">
        <PanelTag code="ADV-01" class="mb-2" />
        <p class="font-display text-xs tracking-wide text-[var(--color-ink-slate)]/70 mb-2">?êÊú¨?àÁ?Ê®°Êì¨?≤Á?</p>
        <div class="h-[360px]">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-4">
        ...
        <div v-if="data.dataLimitations.length" class="panel p-4 border-l-4" style="border-left-color: var(--color-signal-red)">
          ...
          <p class="font-display text-sm mb-2 text-[var(--color-signal-red)]">?™Áõ°‰πãË?</p>
        ...
        <div v-else class="panel p-4 flex items-center justify-center text-sm text-[var(--color-ink-slate)]/60">
          Ë≥áÊ??èÂ?Ë∂≥Ô?Âª∫Ë≠∞‰ø°Â?Á®ãÂ∫¶È´ò„Ä?
        </div>
      </div>
    </template>
  </div>
