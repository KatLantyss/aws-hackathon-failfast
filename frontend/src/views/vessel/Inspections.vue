...
<template>
  <div class="panel p-4 flex flex-col gap-3">
    ...
    <StateDisplay
      ...
      empty-title="Ê≠§ËàπÂ∞öÁÑ°Ê∞¥‰?Ê™¢Êü•Ë®òÈ?"
      empty-hint="Ë´ãËÅØÁµ°È??πÂ?Ë™ø‰∫∫?°Â??í‰?‰∏ÄÊ¨?UWI Ê™¢Êü•??
    />
      ...
      <li v-for="insp in inspections" :key="insp.id" class="chart-divider py-4 first:border-t-0">
        ...
        <div v-if="expandedId === insp.id" class="mt-4 pl-2 grid grid-cols-1 md:grid-cols-[220px_1fr] gap-6">
          <div class="flex flex-col items-center gap-2">
            ...
            <p class="font-data text-[10px] text-[var(--color-ink-slate)]/50">‰æùÂ?Â°ä‰??≤Ê?Á§∫Ê±°?çÁ?Â∫¶Ô?Á§∫Ê?Ôº?/p>
          </div>
          <div class="flex flex-col gap-3">
            <div class="grid grid-cols-2 gap-3 font-data text-sm">
              <div>
                <p class="text-xs font-body text-[var(--color-ink-slate)]/60">Â°óÊ??¥Ê???/p>
                <p>{{ insp.paintBreakdownPct.toFixed(1) }}%</p>
              </div>
              <div>
                <p class="text-xs font-body text-[var(--color-ink-slate)]/60">?∫Ê?Êß≥Á???/p>
                <p>{{ insp.propellerCondition }}</p>
              </div>
              <div>
                <p class="text-xs font-body text-[var(--color-ink-slate)]/60">?ØÂê¶Âª∫Ë≠∞Ê∏ÖÊ?</p>
                <p :class="insp.cleaningRecommended.includes('PRIORITY') ? 'text-[var(--color-signal-red)]' : ''">
                  {{ insp.cleaningRecommended }}
                </p>
              </div>
              <div>
                <p class="text-xs font-body text-[var(--color-ink-slate)]/60">?ßÁ??∏È?</p>
                <p>{{ insp.photoCount }} Âº?/p>
              </div>
            </div>
